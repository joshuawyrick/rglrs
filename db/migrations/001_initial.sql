...
begin
  ...
  if exists(select 1 from audience_rules r join circle_members cm on cm.circle_id=r.subject_id where r.post_id=p_post and r.rule_type='include_circle' and cm.user_id=p_user) then return true; end if;
  if exists(select 1 from audience_rules r join event_members em on em.event_id=r.subject_id where r.post_id=p_post and r.rule_type='include_event' and em.user_id=p_user) then return true; end if;
...
grant execute on function public.is_friend(uuid,uuid), public.can_view_event(uuid,uuid), public.can_view_post(uuid,uuid), public.is_conversation_member(uuid,uuid) to authenticated;
...
create policy "authors manage audience" on audience_rules for all to authenticated using (exists(select 1 from posts p where p.id=post_id and p.author_id=auth.uid())) with check (exists(select 1 from posts p where p.id=post_id and p.author_id=auth.uid()));
...
begin
  ...
  values (new.id, 'user_' || replace(left(new.id::text,8),'-',''), coalesce(new.raw_user_meta_data->>'full_name','New RGLR'));
  return new;
end $$;
