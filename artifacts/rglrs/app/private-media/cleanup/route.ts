...
export async function POST(request: Request) {
  ...
  if (!url || !secret) return NextResponse.json({ error: "Private media is not configured" }, { status: 503 });
  ...
  try {
    ...
    const r2 = getR2();
    let mediaResult: Record<string, unknown> = {};
    let mediaError: unknown = null;
    if (!r2) {
      mediaError = new Error("Private media is not configured");
    } else {
      try {
        mediaResult = await cleanupExpiredUploads(admin, r2.client, r2.bucket);
      } catch (error) {
        mediaError = error;
      }
    }
    ...
    if (mediaError) {
      const errorId = logServerError("private_media.cleanup_failed", mediaError);
      return NextResponse.json({ error: "Media cleanup failed", errorId, ...mediaResult, locationRowsPruned: Number(locationRowsPruned || 0) }, { status: 500 });
    }
